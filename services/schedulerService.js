...
const POLL_INTERVAL_MS =
  parseInt(process.env.SCHEDULER_POLL_INTERVAL_MS) || 15_000; // 15s default
...
async function processHeader(row) {
  const { ID, DEVICE_ID, USER_ID, MESSAGE_TYPE, MESSAGE_TEXT, TEMPLATE_ID } =
    row;
  ...
  const [details] = await db.query(
    "SELECT * FROM scbcdt WHERE ID = ? AND USER_ID = ? AND IS_SENT = 0 AND (NEXT_ATTEMPT_AT IS NULL OR NEXT_ATTEMPT_AT <= NOW()) ORDER BY NOKEY ASC",
    [ID, USER_ID],
  );
  if (!details.length) {
  ...
  if (MESSAGE_TYPE === "template" && TEMPLATE_ID) {
    const [trows] = await db.query(
      "SELECT ID, TEMP_TYPE, TEMP_FILE, TEMP_MESSAGE, TEMP_BUTTONS FROM temptbl WHERE ID = ? AND USER_ID = ?",
      [TEMPLATE_ID, USER_ID],
    );
  ...
  for (const det of details) {
    ...
    if (USE_QUEUE && queueModule) {
      try {
        await queueModule.addRecipientJob({
          headerId: ID,
          nokey,
          deviceId: DEVICE_ID,
          userId: USER_ID,
        ...
        const postponeSeconds = Math.max(
          5,
          Math.ceil(BASE_BACKOFF_SECONDS / 6),
        );
        try {
          await db.query(
            "UPDATE scbcdt SET NEXT_ATTEMPT_AT = DATE_ADD(NOW(), INTERVAL ? SECOND), LAST_ERROR = ? WHERE ID = ? AND NOKEY = ? AND USER_ID = ?",
            [postponeSeconds, e.message, ID, nokey, USER_ID],
          );
        } catch (ex) {}
      }
      continue;
    ...
    if (active >= DEVICE_CONCURRENCY) {
      ...
      try {
        await db.query(
          ...
          [postponeSeconds, ID, nokey, USER_ID],
        );
      } catch (e) {}
      continue;
    }
    ...
    try {
      if (!session || !session.connected) {
        ...
        if (retryCount > MAX_RETRIES) {
          await db.query(
            "UPDATE scbcdt SET IS_SENT = 2, DELIVERY_AT = ?, LAST_ERROR = ?, RETRY_COUNT = ? WHERE ID = ? AND NOKEY = ? AND USER_ID = ?",
            [new Date(), errMsg, retryCount, ID, nokey, USER_ID],
          );
        } else {
          const backoff = Math.pow(2, retryCount - 1) * BASE_BACKOFF_SECONDS;
          await db.query(
            "UPDATE scbcdt SET NEXT_ATTEMPT_AT = DATE_ADD(NOW(), INTERVAL ? SECOND), LAST_ERROR = ?, RETRY_COUNT = ? WHERE ID = ? AND NOKEY = ? AND USER_ID = ?",
            [backoff, errMsg, retryCount, ID, nokey, USER_ID],
          );
        }
        continue;
      }
      ...
      if (MESSAGE_TYPE === "live") {
        ...
        const delayMs =
          Math.floor(Math.random() * (RATE_MAX_MS - RATE_MIN_MS + 1)) +
          RATE_MIN_MS;
        await new Promise((r) => setTimeout(r, delayMs));
        const placeholders = { name: det.CONTACT_NAME || "" };
        const finalMessage = applyPlaceholders(
          MESSAGE_TEXT || "",
          placeholders,
        );
        ...
        try {
          ...
          await sendTemplatedMessage({
            ...
            senderRow,
            receiver,
            templateType: templateRow.TEMP_TYPE,
          });
          await db.query(
            "UPDATE scbcdt SET IS_SENT = 1, DELIVERY_AT = ?, LAST_ERROR = NULL, NEXT_ATTEMPT_AT = NULL WHERE ID = ? AND NOKEY = ? AND USER_ID = ?",
            [new Date(), ID, nokey, USER_ID],
...
if (USE_QUEUE) {
  try {
    queueModule = require("./queueService");
  } catch (e) {
    console.warn(
      "[SCHED] job queue requested but queue module failed to load:",
      e.message,
    );
  }
}
...
async function poll() {
  ...
  try {
    const [rows] = await db.query(
      "SELECT * FROM scbchd WHERE STATUS = 'scheduled' AND DELIVERY_AT <= NOW() ORDER BY DELIVERY_AT ASC LIMIT 50",
    );
    for (const row of rows) {
      try {
        await processHeader(row);
      } catch (e) {
        console.error("[SCHED] processHeader error", e.message);
      }
    }
  } catch (e) {
